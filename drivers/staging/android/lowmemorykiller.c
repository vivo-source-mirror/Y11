				   ...
				   unsigned long action, void *data)
{
	...
	if (pressure >= 95) {
		...
		if ((other_free < lowmem_minfree[array_size - 1]) &&
		    (other_file < vmpressure_file_min + (pressure - 95 + 1) * 20000)) {
...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		...
		/*added by luruichao start*/
		if (unlikely(bsp_test_mode))
			if (!strncmp(tsk->comm, "om.vivo.bsptest", 16) ||
				!strncmp(tsk->comm, "stressapptest", 14)) {
				lowmem_print(3, "skip task %s(pid %d), let it go\n", tsk->comm, tsk->pid);
				continue;
			}
		/*added by luruichao end*/
