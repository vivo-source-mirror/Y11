...
void mdss_check_dsi_ctrl_status(struct work_struct *work, uint32_t interval)
{
	...
	MDSS_XLOG(mipi->mode);
	if ((lcd_need_reset) || (lcd_need_recovery == 1)) {
		ret = 0;	/*tp notify ft8006p driver ic gone bad, need to reset*/
	} else if (vivo_esd_check_ps_status == 1) {
		mdss_mdp_clk_ctrl(MDP_BLOCK_POWER_ON);
		ret = ctrl_pdata->check_status(ctrl_pdata);
		mdss_mdp_clk_ctrl(MDP_BLOCK_POWER_OFF);
	} else {
		ret = 1;	/* ESD check return OK when vivo_esd_check_ps_status = 0 */
	}
