...
static unsigned int panel_reset_status; /* 0: power off ; 1: power on*/
extern unsigned int is_atboot;

/* indicate panel off state */
unsigned int panel_off_state; /* 0: panel not off,  1:panel off*/

unsigned int vddio_off_enable;
...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
	 ...
	 if (phone_shutdown_state || (lcm_software_id == 0x00) || lcd_need_reset || lcd_need_recovery) {
		/***********keep reset high when power off***********/
		pr_err("%s:is_atboot = %d, phone_shutdown_state = %d, lcm_software_id = %x\n",
			__func__, is_atboot, phone_shutdown_state, lcm_software_id);
		vddio_off_enable = 1;
...
int incell_avdd_avee_output(int status)
{
	...
	pr_err("%s: power ctrl parameter: enable is %d, reset_state is %d,panel off state=%d\n", __func__,  status,  panel_reset_status, panel_off_state);

	if ((panel_off_state == 1) || (panel_reset_status == 0)) {
		pr_err("%s: Invalid panel state, only unblank status can use\n", __func__);
		return -EINVAL;
	}

	ret = msm_mdss_enable_vreg(/*PMIC AVDD AVEE */
			ctrl_pdata->panel_power_data.vreg_config+1,
			ctrl_pdata->panel_power_data.num_vreg-1, status);
	return ret;
}
EXPORT_SYMBOL(incell_avdd_avee_output);
...
int mdss_dsi_panel_reset_and_powerctl(int enable)
{
	...
	LCM_INFO("power ctrl parameter: enable is %d, reset_state is %d,panel off state=%d, AT mode = %d\n", enable,  panel_reset_status, panel_off_state, is_atboot);

	if ((!panel_off_state) && (enable < INCELL_LCM_TP_RST_LOW)) {
		pr_err("%s: Invalid panel state, only blank power off can use\n", __func__);
		return -EINVAL;
	}

	if (enable < 0 || (enable == panel_reset_status)) {
		return -EINVAL;
	}

	if (enable == INCELL_LCM_POWER_ON) {
		ret = mdss_dsi_panel_power_on(pdata);
		ret = mdss_dsi_panel_reset(pdata, INCELL_LCM_POWER_ON);
	} else if (enable == INCELL_LCM_POWER_OFF) {
		phone_shutdown_state = 1;
		ret = mdss_dsi_panel_power_off(pdata);
	} else if (enable == INCELL_LCM_POWER_ON_WITHOUT_LCM_RST) {
		ret = mdss_dsi_panel_power_on(pdata);
		ret = mdss_dsi_panel_reset(pdata, INCELL_LCM_POWER_ON_WITHOUT_LCM_RST);
	} else if (enable == INCELL_LCM_TP_RST_LOW) {
		incell_tp_reset_output(0);
	} else if (enable == INCELL_LCM_TP_RST_HIGH) {
		incell_tp_reset_output(1);
	} else if (enable == INCELL_LCM_AVDD_AVEE_OFF) {
		incell_avdd_avee_output(0);
	} else if (enable == INCELL_LCM_AVEE_AVEE_ON) {
		incell_avdd_avee_output(1);
	}
	return ret;
}
...
static int mdss_dsi_off(struct mdss_panel_data *pdata, int power_state)
{
	...
	static bool first_panel_flag = true;
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	if (!mipi->lp11_init) {
		/*ft8006p do not need to reset twice*/
 ...
 */
int lcm_software_id;
static char  *lcm_id_str = ":lcm_software_id=0x";
static char lcm_software_id_str[30] = {0};

int lcm_max_brightness;
static char  *lcm_max_brightness_str = ":lcm_max_brightness=0x";
static char lcm_max_backlight_str[30] = {0};
extern int panel_max_bl_level;
		...
		struct platform_device *pdev, char *panel_cfg)
{
	...
	char *lcmid = NULL, *lcmbacklight = NULL;
	...
	//hw_subtype = get_bbk_hw_subtype();
	pr_err("%s: hw_subtype is %d 0911 1809 \n", __func__, hw_subtype);
	...
	if (!len) {
		...
		if (!str2) {
			...
			lcmid = strnstr(panel_cfg, lcm_id_str, len);
			if (lcmid) {
				lcmid += strlen(lcm_id_str);
				strlcpy(lcm_software_id_str, lcmid, 3);
				lcm_software_id = str2int(lcm_software_id_str);
			} else {
				lcm_software_id = 0xFF;  /* errid: indicate no lcm softid send from LK*/
			}
			/*** add for lcm id end ***/

			/*max lcm backlight*/
			lcmbacklight = strnstr(panel_cfg, lcm_max_brightness_str, len);
			if (lcmbacklight) {
				lcmbacklight += strlen(lcm_max_brightness_str);
				strlcpy(lcm_max_backlight_str, lcmbacklight, 3);
				lcm_max_brightness = str2int(lcm_max_backlight_str);
			} else {
				lcm_max_brightness = 0x00;  /* errid: indicate no lcm softid send from LK*/
			}
		}
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->disp_vddio_en = of_get_named_gpio(
			ctrl_pdev->dev.of_node,
			"qcom,platform-enable-vddioen-gpio", 0);

	if (!gpio_is_valid(ctrl_pdata->disp_vddio_en))
			pr_err("%s:%d, lcm 1.8 vddio en enable gpio not specified\n",
					__func__, __LINE__);
