...
static u32 kpdpwr_monitor_timeout = 6;
		...
		struct qpnp_pon_config *cfg, bool debug_mode)
{
	...
	if (rc)
		pr_err("%s %d Unable to config pon reset\n",
				__func__, __LINE__);
	else
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	cfg = qpnp_get_cfg(pon, PON_KPDPWR_RESIN);
	if (!cfg) {
		pr_err("%s %d PON_RESIN is NULL\n",
				__func__, __LINE__);
		return -EINVAL;
	}
	rc = qpnp_pon_kpdpwr_resin_cfg(pon, cfg, !!value);

	return size;
}

static DEVICE_ATTR(voldown_status, 0664, qpnp_pon_kpdpwr_resin_show, qpnp_pon_kpdpwr_resin_store);

static ssize_t qpnp_pon_resin_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	int rc;
	...
	return snprintf(buf, sizeof(key_status), "%d\n", key_status);
}

static ssize_t qpnp_pon_resin_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	u32 value;
	...
	if (0 == value) {
		rc = qpnp_pon_masked_write(pon, QPNP_PON_INT_EN_CLR(pon),
				QPNP_PON_RESIN_INT, 0x2);
		if (rc)
			dev_err(dev, "Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_INT_EN_CLR(pon), rc);

		rc =  qpnp_pon_masked_write(pon, QPNP_PON_PULL_CTL(pon),
				QPNP_PON_RESIN_PULL_UP, 0);
		if (rc)
			dev_err(dev, "Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_PULL_CTL(pon), rc);
	} else {
		qpnp_pon_masked_write(pon, QPNP_PON_PULL_CTL(pon),
				QPNP_PON_RESIN_PULL_UP, 1);
		if (rc)
			dev_err(dev, "Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_PULL_CTL(pon), rc);

		qpnp_pon_masked_write(pon, QPNP_PON_INT_EN_SET(pon),
				QPNP_PON_RESIN_INT, 0x2);
		if (rc)
			dev_err(dev, "Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_INT_EN_SET(pon), rc);
	}

	return size;
}

static DEVICE_ATTR(resin_status, 0644, qpnp_pon_resin_show, qpnp_pon_resin_store);
...
void qpnp_pon_kpdpwr_resin_ex(int enable)
{
	...
	rc = qpnp_pon_kpdpwr_resin_cfg(pon, cfg, !!enable);
...
static void kpdpwr_monitor_work_func(struct work_struct *work)
{
	...
	if (!kpdpwr_cfg) {
		dev_err(&(to_spmi_device(pon->pdev->dev.parent)->dev), "Invalid kpdpwr_cfg pointer\n");
		return;
	}
	/*modify state*/
	kpdpwr_monitor_state = 1;
	pr_err("%s\n", monitor_state[0]);
	kobject_uevent_env(&(to_spmi_device(pon->pdev->dev.parent)->dev.kobj), KOBJ_CHANGE, monitor_state);
}
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	if (pon_rt_sts == (QPNP_PON_KPDPWR_N_SET | QPNP_PON_RESIN_N_SET))
		schedule_delayed_work(&pon->kpdpwr_work,
				kpdpwr_monitor_timeout*HZ);
	else {
		cancel_delayed_work(&pon->kpdpwr_work);
	}
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(&pdev->dev, &dev_attr_voldown_status);
	if (rc) {
		dev_err(&pdev->dev, "sys file voldown_status creation failed\n");
		return rc;
	}

	rc = device_create_file(&pdev->dev, &dev_attr_resin_status);
	if (rc) {
		dev_err(&pdev->dev, "sys file resin_status creation failed\n");
		return rc;
	}
...
static int qpnp_pon_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_voldown_status);
	device_remove_file(&pdev->dev, &dev_attr_resin_status);
