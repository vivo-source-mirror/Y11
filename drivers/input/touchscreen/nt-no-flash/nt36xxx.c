...
#if NVT_TOUCH_ESD_PROTECT
struct delayed_work nvt_spi_esd_check_work;
struct workqueue_struct *nvt_spi_esd_check_wq;
unsigned long irq_timer = 0;
uint8_t spi_esd_check = false;
uint8_t spi_esd_retry = 0;
#endif /* #if NVT_TOUCH_ESD_PROTECT */
...
uint32_t SWRST_N8_ADDR_2ND = 0; //read from dtsi
...
static struct mtk_chip_config mtk_nt_chip_info = {
	.rx_mlsb = 1,
	.tx_mlsb = 1,
	.cs_pol = 0,
	.sample_sel = 0,	
...
static inline int32_t spi_read_write(struct spi_device *client, uint8_t *buf, size_t len , NVT_SPI_RW rw)
{
	...
	memcpy(ts_spi->xbuf, buf, len);
	switch (rw) {
		case NVTREAD:
			//t.tx_buf = &buf[0];
			t.tx_buf = &ts_spi->xbuf[0];			
			t.rx_buf = ts_spi->rbuf;
			t.len    = (len + DUMMY_BYTES);
			break;

		case NVTWRITE:
			t.tx_buf = ts_spi->xbuf;
			break;
	}
...
void nvt_boot_ready(void)
{
	...
	if (!ts_spi->hw_crc) {
		//---write BOOT_RDY status cmds---
		nvt_write_addr(ts_spi->mmap->BOOT_RDY_ADDR, 0);

		//---write POR_CD cmds---
		nvt_write_addr(ts_spi->mmap->POR_CD_ADDR, 0xA0);
	}
}
...
void nvt_sw_reset(void)
{
	...
	nvt_write_addr(SWRST_N8_ADDR_2ND, 0x55);
...
void nvt_sw_reset_idle_Spi(void)
{
	...
	nvt_write_addr(SWRST_N8_ADDR_2ND, 0xAA);
...
void nvt_bootloader_reset_Spi(void)
{
	...
	nvt_write_addr(SWRST_N8_ADDR_2ND, 0x69);
...
static ssize_t nvt_flash_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
	...
	spi_nvt_spi_esd_check_enable(false);
#endif /* #if NVT_TOUCH_ESD_PROTECT */
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	if (IS_ERR(ts_spi->pinctrl_gpios)) {
        ...
        printk("can not find touch ts_spi->pinctrl_gpios");
        return ret;
    }*/
	...
	if (IS_ERR(ts_spi->spi_cs_set))	{
		...
		NVT_ERR("%s ts_spi->spi_cs_set pinctrl\n", __func__);		
	}
	pinctrl_select_state(ts_spi->pinctrl_gpios, ts_spi->spi_cs_set);

	ts_spi->pin_miso_pulllow = pinctrl_lookup_state(ts_spi->pinctrl_gpios, "spi_cs_pulllow");
	if (IS_ERR(ts_spi->pin_miso_pulllow))	{
		ret = PTR_ERR(ts_spi->pin_miso_pulllow);
		NVT_ERR("%s ts_spi->pin_miso_pulllow pinctrl\n", __func__);		
	}

	ts_spi->pin_int_low = pinctrl_lookup_state(ts_spi->pinctrl_gpios, "ts_int_active");
	if (IS_ERR(ts_spi->pin_int_low))	{
		ret = PTR_ERR(ts_spi->pin_int_low);
		NVT_ERR("%s ts_spi->pin_int_low pinctrl\n", __func__);		
	}*/
	...
	ret = of_property_read_u32(np, "novatek,swrst-n8-addr-2nd", &SWRST_N8_ADDR_2ND);
	if (ret) {
		NVT_ERR("error reading novatek,swrst-n8-addr-2nd. ret=%d\n", ret);
		return ret;
	} else {
		NVT_LOG("SWRST_N8_ADDR_2ND=0x%06X\n", SWRST_N8_ADDR_2ND);
	}
...
#if NVT_TOUCH_ESD_PROTECT
void spi_nvt_spi_esd_check_enable(uint8_t enable)
{
	/* update interrupt timer */
	irq_timer = jiffies;
	/* clear spi_esd_retry counter, if protect function is enabled */
	spi_esd_retry = enable ? 0 : spi_esd_retry;
	/* enable/disable esd check flag */
	spi_esd_check = enable;
}
...
static void nvt_spi_esd_check_func(struct work_struct *work)
{
	...
	//NVT_LOG("spi_esd_check = %d (retry %d)\n", spi_esd_check, spi_esd_retry);	//DEBUG

	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && spi_esd_check) {
...
static int32_t nvt_ts_point_data_checksum(uint8_t *buf, uint8_t length)
{
	...
	if (checksum != buf[length]) {
		VTD("i2c/spi packet checksum not match. (point_data[%d]=0x%02X, checksum=0x%02X)\n", (length), buf[length], checksum);

		for (i = 0; i < 10; i++) {
			VTD("%02X %02X %02X %02X %02X %02X\n", buf[1+i*6], buf[2+i*6], buf[3+i*6], buf[4+i*6], buf[5+i*6], buf[6+i*6]);
		}
...
*/
   ...
   if (nvt_wdt_fw_recovery(point_data)) {
	...
	if (nvt_fw_recovery(point_data)) {
		spi_nvt_spi_esd_check_enable(true);
		goto XFER_ERROR;
	}
#endif /* #if NVT_TOUCH_ESD_PROTECT */
	...
	for (retry = 5; retry > 0; retry--) {
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			if (found_nvt_chip) {
				...
				ts_spi->carrier_system = trim_id_table[list].hwinfo->carrier_system;
				ts_spi->hw_crc = trim_id_table[list].hwinfo->hw_crc;
				ret = 0;
				goto out;
			} else {
				ts_spi->mmap = NULL;
...
extern int8_t nvt_customizeCmd_Wait(uint8_t u8WaitAddr, uint8_t u8WaitStatus);
...
static int bbk_xxx_mode_change(int which) {
	...
	if(which == TOUCHSCREEN_GESTURE) {
		...
		if (last_touch_state == TOUCHSCREEN_SLEEP) {
			...
			VTI("callingstate %d ", vtsData->callingstate);
			if (vtsData->callingstate == 0) {
				//nvt_ts_resume(&ts_spi->client->dev);
				mutex_lock(&ts_spi->lock);
				nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME, FWTYPE_Normal, FWTYPE_REQUEST_NO);
				...
				if (ts_spi->wkg_info) {
					//nvt_customizeCmd_Wait(EVENT_MAP_RESET_COMPLETE, RESET_STATE_INIT);
	        		nvt_update_gesture_firmware(BOOT_UPDATE_FIRMWARE_NAME, FWTYPE_Normal, FWTYPE_REQUEST_NO);
				} else {
	...
	if(which == TOUCHSCREEN_SLEEP) {
		...
		mutex_lock(&(vivoTsGetVtsData()->lcmResumeAndProximityMutex));	
		#ifdef CONFIG_OF
		//pinctrl_select_state(ts_spi->pinctrl_gpios, ts_spi->pin_int_low);
		//pinctrl_select_state(ts_spi->pinctrl_gpios, ts_spi->pin_miso_pulllow);		
		#endif
...
static int bbk_xxx_module_id(void)
{
	...
	return VTS_MVC_BOE; /* default boe*/
}
...
extern int bbk_slsi_get_rawordiff_data_spi(int which, int *data);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	vts_data->proximityRtState = 1;
	vts_data->lastProxState = 1;
	vts_data->gestureMode = 0;
	vts_data->needChangeToGestureMode = 1;
	...
	vts_data->getRawOrDiffData = bbk_slsi_get_rawordiff_data_spi;	
	...
	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1901_no_flash_fw, sizeof(PD1901_no_flash_fw), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x3a);
	vivoTsFwAdd(VTS_FW_TYPE_MP, PD1901_no_flash_fw_mp, sizeof(PD1901_no_flash_fw_mp), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x3a);

	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1901_no_flash_fw_lc_boe, sizeof(PD1901_no_flash_fw_lc_boe), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x51);
	vivoTsFwAdd(VTS_FW_TYPE_MP, PD1901_no_flash_fw_lc_boe_mp, sizeof(PD1901_no_flash_fw_lc_boe_mp), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x51);

	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1901_no_flash_fw_inx, sizeof(PD1901_no_flash_fw_inx), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x61);
	vivoTsFwAdd(VTS_FW_TYPE_MP, PD1901_no_flash_fw_inx_mp, sizeof(PD1901_no_flash_fw_inx_mp), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x61);

	vivoTsFwAdd(VTS_FW_TYPE_FW, PD1901_no_flash_fw_36525b_boe, sizeof(PD1901_no_flash_fw_36525b_boe), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x3b);
	vivoTsFwAdd(VTS_FW_TYPE_MP, PD1901_no_flash_fw_36525b_boe_mp, sizeof(PD1901_no_flash_fw_36525b_boe_mp), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_BOE, 0x3b);
	
	ts_spi = kzalloc(sizeof(struct nvt_ts_data), GFP_KERNEL | GFP_DMA);
	if (ts_spi == NULL) {
		NVT_ERR("failed to allocated memory for nvt ts data\n");
		return -ENOMEM;
	}
	ts_spi->xbuf = (uint8_t *)kzalloc((NVT_TANSFER_LEN+1), GFP_KERNEL | GFP_DMA);
	if (ts_spi->xbuf == NULL) {
	...
	//ts_spi->client->controller_data = (void *)&mtk_nt_chip_info;
	VTI("mode=%d\n", ts_spi->client->mode);
...
if(0) {
	...
	nvt_spi_esd_check_wq = create_workqueue("nvt_spi_esd_check_wq");
	queue_delayed_work(nvt_spi_esd_check_wq, &nvt_spi_esd_check_work,
			msecs_to_jiffies(NVT_TOUCH_ESD_CHECK_PERIOD));
#endif /* #if NVT_TOUCH_ESD_PROTECT */
...
return:
	Executive outcomes. 0---succeed.
*******************************************************/
static int32_t spi_nvt_ts_remove(struct spi_device *client)
{
#if defined(CONFIG_FB)
...
static int32_t __init nvt_driver_init(void)
{
	...
	ret = report_lcm_id();
	VTI("Novatek 36525 0-flash module lcm id is 0x%x", ret);
	if ((ret != 0x3a) && (ret != 0x3b) && (0x51 != ret) && (0x61 != ret)) {
		VTI("not spi module");
		return 0;	
	}
