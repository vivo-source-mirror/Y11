...
extern struct delayed_work nvt_spi_esd_check_work;
...
int bbk_slsi_get_rawordiff_data_spi(int which, int *data)
{
	int32_t i = 0;
	int32_t j = 0;	
...
int8_t nvt_customizeCmd(uint8_t u8Cmd)
{
	...
	#endif
	mutex_lock(&ts_spi->lock); 	//[20190724,jx]Rectify!
	...
	mutex_unlock(&ts_spi->lock);//[20190724,jx]Rectify!
	NVT_LOG("--\n");
	return ret;
}
...
/*int8_t nvt_customizeCmd_Wait(uint8_t u8WaitAddr, uint8_t u8WaitStatus)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
	const int32_t retry = 50;
