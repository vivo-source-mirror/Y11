...
extern void nvt_change_mode_Spi(uint8_t mode);
extern uint8_t nvt_get_fw_pipe_Spi(void);
extern void nvt_read_mdata_Spi(uint32_t xdata_addr, uint32_t xdata_btn_addr);
extern void nvt_get_mdata_Spi(int32_t *buf, uint8_t *m_x_num, uint8_t *m_y_num);
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	nvt_get_mdata_Spi(xdata, &x_num, &y_num);
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	if (nvt_get_fw_pipe_Spi() == 0)
		raw_pipe_addr = ts_spi->mmap->RAW_PIPE0_ADDR;
	else
		raw_pipe_addr = ts_spi->mmap->RAW_PIPE1_ADDR;
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		...
		CTP_SPI_READ(ts_spi->client, buf, IC_X_CFG_SIZE * 2 + 1);
		memcpy(rawdata_buf + y * IC_X_CFG_SIZE * 2, buf + 1, IC_X_CFG_SIZE * 2);
	}
#if TOUCH_KEY_NUM > 0
	if (nvt_get_fw_pipe_Spi() == 0)
		raw_btn_pipe_addr = ts_spi->mmap->RAW_BTN_PIPE0_ADDR;
	else
		raw_btn_pipe_addr = ts_spi->mmap->RAW_BTN_PIPE1_ADDR;

	//---change xdata index---
	nvt_set_page(raw_btn_pipe_addr);

	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	CTP_SPI_READ(ts_spi->client, buf, IC_KEY_CFG_SIZE * 2 + 1);
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		...
		CTP_SPI_READ(ts_spi->client, buf, X_Channel * 2 + 1);
	...
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	CTP_SPI_READ(ts_spi->client, buf, Key_Channel * 2 + 1);
...
const struct seq_operations nvt_selftest_seq_ops_Spi = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
{
	struct device_node *np = ts_spi->client->dev.of_node;
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		 ...
		 */
		snprintf(mpcriteria, PAGE_SIZE, "novatek-mp-criteria-%04X", ts_spi->nvt_pid);
	...
	if (nvt_check_fw_reset_state_Spi(RESET_STATE_NORMAL_RUN)) {
		mutex_unlock(&ts_spi->lock);
	...
	nvt_change_mode_Spi(MP_MODE_CC);
	...
	return seq_open(file, &nvt_selftest_seq_ops_Spi);
}
...
return:
	n.a.
*******************************************************/
int32_t nvt_mp_parse_ain_Spi(struct device_node *np, const char *name, uint8_t *array, int32_t size)
{
...
return:
	n.a.
*******************************************************/
int32_t nvt_mp_parse_u32_Spi(struct device_node *np, const char *name, int32_t *para)
{
	int32_t ret;
...
return:
	n.a.
*******************************************************/
int32_t nvt_mp_parse_array_Spi(struct device_node *np, const char *name, int32_t *array,
		int32_t size)
{
	struct property *data;
	int32_t len, ret;
...
static int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	/* MP Config*/
	if (nvt_mp_parse_u32_Spi(np, "IC_X_CFG_SIZE", &IC_X_CFG_SIZE))
		return -1;

	if (nvt_mp_parse_u32_Spi(np, "IC_Y_CFG_SIZE", &IC_Y_CFG_SIZE))
		return -1;

#if TOUCH_KEY_NUM > 0
	if (nvt_mp_parse_u32_Spi(np, "IC_KEY_CFG_SIZE", &IC_KEY_CFG_SIZE))
		return -1;
#endif

	if (nvt_mp_parse_u32_Spi(np, "X_Channel", &X_Channel))
		return -1;

	if (nvt_mp_parse_u32_Spi(np, "Y_Channel", &Y_Channel))
		return -1;

	if (nvt_mp_parse_ain_Spi(np, "AIN_X", AIN_X, IC_X_CFG_SIZE))
		return -1;

	if (nvt_mp_parse_ain_Spi(np, "AIN_Y", AIN_Y, IC_Y_CFG_SIZE))
		return -1;

#if TOUCH_KEY_NUM > 0
	if (nvt_mp_parse_ain_Spi(np, "AIN_KEY", AIN_KEY, IC_KEY_CFG_SIZE))
		return -1;
#endif

	/* MP Criteria */
	if (ts_spi->carrier_system) {
		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Short_Diff_P", PS_Config_Lmt_Short_Diff_P,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Short_Diff_N", PS_Config_Lmt_Short_Diff_N,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Short_Base_P", PS_Config_Lmt_Short_Base_P,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Short_Base_N", PS_Config_Lmt_Short_Base_N,
				X_Channel * Y_Channel + Key_Channel))
			return -1;
	} else {
		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Short_Rawdata_P", PS_Config_Lmt_Short_Rawdata_P,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Short_Rawdata_N", PS_Config_Lmt_Short_Rawdata_N,
				X_Channel * Y_Channel + Key_Channel))
			return -1;
	}

	if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Open_Rawdata_P", PS_Config_Lmt_Open_Rawdata_P,
			X_Channel * Y_Channel + Key_Channel))
		return -1;

	if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_Open_Rawdata_N", PS_Config_Lmt_Open_Rawdata_N,
			X_Channel * Y_Channel + Key_Channel))
		return -1;

	if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_Rawdata_P", PS_Config_Lmt_FW_Rawdata_P,
			X_Channel * Y_Channel + Key_Channel))
		return -1;

	if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_Rawdata_N", PS_Config_Lmt_FW_Rawdata_N,
			X_Channel * Y_Channel + Key_Channel))
		return -1;

	if (ts_spi->carrier_system) {
		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_CC_I_P", PS_Config_Lmt_FW_CC_I_P,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_CC_I_N", PS_Config_Lmt_FW_CC_I_N,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_CC_Q_P", PS_Config_Lmt_FW_CC_Q_P,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_CC_Q_N", PS_Config_Lmt_FW_CC_Q_N,
				X_Channel * Y_Channel + Key_Channel))
			return -1;
	} else {
		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_CC_P", PS_Config_Lmt_FW_CC_P,
				X_Channel * Y_Channel + Key_Channel))
			return -1;

		if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_CC_N", PS_Config_Lmt_FW_CC_N,
				X_Channel * Y_Channel + Key_Channel))
			return -1;
	}

	if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_Diff_P", PS_Config_Lmt_FW_Diff_P,
			X_Channel * Y_Channel + Key_Channel))
		return -1;

	if (nvt_mp_parse_array_Spi(np, "PS_Config_Lmt_FW_Diff_N", PS_Config_Lmt_FW_Diff_N,
			X_Channel * Y_Channel + Key_Channel))
		return -1;

	if (nvt_mp_parse_u32_Spi(np, "PS_Config_Diff_Test_Frame", &PS_Config_Diff_Test_Frame))
		return -1;
...
*******************************************************/
int32_t nvt_mp_proc_init_Spi(void)
{	
