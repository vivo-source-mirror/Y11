...
static int getDiffOrRawData(int which, char *buf)
{
	...
	log_buf = (unsigned char *)kzalloc(1024*sizeof(unsigned char), GFP_KERNEL);
	if (log_buf == NULL) {
		VTI("kzalloc log_buf fail.");
		kfree(rawOrDiffData);
		ret = -1;
		return ret;
	}
...
exit:
	if (ret > (4095 - 7)) {
		for (i = 0; i < txNum; i++) {
			memset(log_buf, 0, 1024*sizeof(unsigned char));
			offset = 0;
			for (j = 0; j < rxNum; j++) {
					...
					struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (vivo_ts_data->hasFingerGesture) {
		...
		if (fingerGestureTimeoutFilter() == 1) {
			...
			if (val == 0) {
				vivo_ts_data->commonGestureOff = 1;	/* include dclick/@/f... */
			} else {
