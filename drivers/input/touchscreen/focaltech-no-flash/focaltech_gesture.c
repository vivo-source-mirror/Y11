...
struct fts_gesture_st {
    ...
    u16 coordinate_x[FTS_GESTURE_POINTS_MAX_VIVO];
    u16 coordinate_y[FTS_GESTURE_POINTS_MAX_VIVO];
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
    ...
    memset(fts_gesture_data.coordinate_x, 0, FTS_GESTURE_POINTS_MAX_VIVO * sizeof(u16));
    memset(fts_gesture_data.coordinate_y, 0, FTS_GESTURE_POINTS_MAX_VIVO * sizeof(u16));
	...
	if (pointnum < FTS_GESTURE_POINTS_MAX_VIVO) {
		for (i = pointnum; i < FTS_GESTURE_POINTS_MAX_VIVO; i++) {
			fts_gesture_data.coordinate_x[i] = 65535;
			fts_gesture_data.coordinate_y[i] = 65535;
		}
	
		if( GESTURE_O_CW == gestrue_id )
			fts_gesture_data.coordinate_y[FTS_GESTURE_POINTS_MAX_VIVO-1] = 16;
		else
			fts_gesture_data.coordinate_y[FTS_GESTURE_POINTS_MAX_VIVO-1] = 32;
	
		 fts_gesture_data.header[1] = FTS_GESTURE_POINTS_MAX_VIVO;
	 }

	
	VTI("[GESTURE]%d read gestrue_id", gestrue_id);
...
void fts_gesture_recovery(void)
{
    if ((ENABLE == fts_gesture_data.mode) && (ENABLE == fts_gesture_data.active)) {
        VTD("enter fts_gesture_recovery");
        fts_write_reg_byte(0xD1, 0xff);
        fts_write_reg_byte(0xD2, 0xff);
        fts_write_reg_byte(0xD5, 0xff);
        fts_write_reg_byte(0xD6, 0xff);
        fts_write_reg_byte(0xD7, 0xff);
        fts_write_reg_byte(0xD8, 0xff);
        fts_write_reg_byte(FTS_REG_GESTURE_EN, ENABLE);
    }
}

int fts_gesture_buf_vivo(u16 *data)
{
	int i = 0;
...
void fts_gesture_mode_set(bool mode)
{
...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
	...
	fts_read_reg_byte(FTS_REG_FW_TYPE, &fw_type);
	VTI("read fw type(0xB4):0x%02x", fw_type);
    ...
    for (i = 0; i < 5; i++) {
        ...
        */
        fts_write_reg_byte(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        fts_read_reg_byte(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int fts_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    if (fts_gesture_data.active == DISABLE) {
        VTD("gesture in suspend is failed, no running fts_gesture_resume");
        return -EINVAL;
    }

    fts_gesture_data.active = DISABLE;
    for (i = 0; i < 5; i++) {
        fts_write_reg_byte(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        fts_read_reg_byte(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
