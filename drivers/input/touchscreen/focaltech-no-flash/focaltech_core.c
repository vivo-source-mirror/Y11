...
const unsigned char VIVO_CUST_FW_FT8006P_SPI_TRULY[] =
{
#include FTS_UPGRADE_FW_FILE
};

const unsigned char VIVO_CUST_FW_FT8006P_SPI_TRULY_BOE[] =
{
#include FTS_UPGRADE_FW2_FILE
};

const unsigned char VIVO_CUST_FW_FT8006P_SPI_TRULY_AUO[] =
{
#include FTS_UPGRADE_FW3_FILE
};

const unsigned char VIVO_CUST_FW_FT8006P_SPI_TRULY_LC[] =
{
...
//extern void incell_tp_reset_output_paneloff(int status);
...
static int fts_input_alloc(struct fts_ts_data *ts_data)
{
    ...
    ts_data->pnt_buf_size = point_num * FTS_ONE_TCH_LEN + 3 + FTS_GESTURE_DATA_LEN;
...
static int focaltech_mode_change(int which)
{
	...
	if (which == TOUCHSCREEN_SLEEP) {
		...
		if (last_ts_state == TOUCHSCREEN_NORMAL) {
			...
			mdelay(5);
			mdss_dsi_panel_reset_and_powerctl(5);
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	vts_data->processByPackage = bbk_ft_process_by_package;
	...
	vts_data->RFTestKey = "com.focaltech.deltadiff:BspTestActivity:com.focaltech.deltadiff:0:testResult";

	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x15);
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY_BOE, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY_BOE), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x3C);
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY_AUO, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY_AUO), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x40);
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY_LC, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY_LC), "PD1930", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x50);
	...
	spi->mode = SPI_MODE_1;
	spi->bits_per_word = 8;
	spi->max_speed_hz = FTS_SPI_SPEED;
	if (!spi->max_speed_hz)
...
static int __init fts_ts_init(void)
{
	...
	if ((0x15 != report_lcm_id()) && (0x3C != report_lcm_id()) && (0x40 != report_lcm_id()) && (0x50 != report_lcm_id())) {
		VTI("not 8006P");
		return 0;
	}
