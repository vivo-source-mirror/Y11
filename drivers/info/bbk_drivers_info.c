...
static char bbk_dts_subtype[16] = "subtype_null";
...
#define BBK_USE_CMDLINE_SUBTYPE

int get_bbk_hw_subtype(void)
{
#ifdef BBK_USE_CMDLINE_SUBTYPE

	int sub_type = 0;
	if (!kstrtoint(bbk_dts_subtype, 10, &sub_type)) {
		return sub_type;
	} else {
		return -EINVAL;
	};
...
static __init int set_bbk_dts_subtype(char *str)
{
	strlcpy(bbk_dts_subtype, str, sizeof(bbk_dts_subtype));
	pr_err("bbk dts subtype is %s\n", bbk_dts_subtype);
	return 0;
}
early_param("subtype", set_bbk_dts_subtype);
...
static ssize_t curr_dts_subtype_show(struct kobject *kobj,
...
static struct debug_sysfs_entry curr_dts_subtype = 
    __ATTR(curr_dts_subtype, S_IRUGO, 
			curr_dts_subtype_show, NULL);
