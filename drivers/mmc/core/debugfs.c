...
static int mmc_ext_csd_open(struct inode *inode, struct file *filp)
{
	...
	for (i = 0; i < 512; i++) {
		if (i%16 == 0) {
			n += snprintf(buf + n, 1024, "0x%04x: %02x ", i, ext_csd[i]);
		} else if (i%16 == 15) {
			n += snprintf(buf + n, 1024, "%02x\n", ext_csd[i]);
		} else {
			n += snprintf(buf + n, 1024, "%02x ", ext_csd[i]);
		}
	}
...
#ifdef CONFIG_MMC_VSM
	if (mmc_card_mmc(card))
		if (!debugfs_create_file("nand_info", S_IRUSR | S_IRGRP | S_IROTH, root, card,
