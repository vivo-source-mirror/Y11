...
__acquires(udc->lock)
{
	...
	for (i = 0; i < hw_ep_max; i++) {
		...
		if ((req.bRequestType & USB_RECIP_MASK) == USB_RECIP_DEVICE)
			...
			udc->dbg_ep0 = false;
		if (udc->dbg_ep0) {
			pr_info("%s: setup packet process start\n", __func__);
			pr_info("control req%02x.%02x v%04x i%04x l%d\n",
...
delegate:
		...
		if (udc->dbg_ep0)
			pr_info("%s: setup packet process end: %d\n", __func__, err);
...
/*vivo add*/
static void usb_enumerate_confdoned(void)
{
	...
	if (usb_psy) {
		pr_err("%s:##### usb confdoned successfully! #####\n", __func__);
		pval.intval = true;
		power_supply_set_property(usb_psy,
                POWER_SUPPLY_PROP_USB_POWERUP_CONFIGED, &pval);
	} else {
