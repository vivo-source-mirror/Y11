...
static enum dual_role_property usb_dr_properties[] = {
...
static int usb_dr_get_property(struct dual_role_phy_instance *dual_role,
			       enum dual_role_property prop, unsigned int *val)
{
	struct msm_otg *motg = dual_role_get_drvdata(dual_role);
	int mode = 2, pr = 2, dr = 2;

	if (!motg)
		return -ENODEV;

	if (motg->start_peripheral) {
	...
	case DUAL_ROLE_PROP_DR:
		*val = dr;
		break;
	default:
		pr_err("%s: %d: dual role class get property %d not supported\n",
		       __func__, __LINE__, prop);
		return -EINVAL;
	}
	pr_info("%s: %d: dual role class get property %d = %d\n",
		__func__, __LINE__, prop, *val);

	return 0;
}

static int usb_dr_prop_writeable(struct dual_role_phy_instance *dual_role,
...
static const struct dual_role_phy_desc dr_desc = {
	.name = "otg_default",
	.supported_modes = DUAL_ROLE_SUPPORTED_MODES_UFP,
	.properties = usb_dr_properties,
	.num_properties = ARRAY_SIZE(usb_dr_properties),
	.get_property = usb_dr_get_property,
	.property_is_writeable = usb_dr_prop_writeable,
};

static int usb_init_dual_role_class(struct msm_otg *motg)
{
	int ret = 0;

	motg->dual_role = devm_dual_role_instance_register(&motg->pdev->dev, &dr_desc);
	if (IS_ERR(motg->dual_role)) {
		ret = PTR_ERR(motg->dual_role);
		pr_err("%s: %d: Couldn't register dual role class %d\n",
		       __func__, __LINE__, ret);
	} else {
		motg->dual_role->drv_data = motg;
	}
...
static void ulpi_init(struct msm_otg *motg)
{
	...
	if (override_phy_init) {
		...
		/*USB eye diagram parameter separation added by @wutianwen 2017.10.08 */
		pr_info("%s:USB state is %s work\n", __func__, usb_otg_state_string(otg->state));
		switch (otg->state) {
		case OTG_STATE_UNDEFINED:
			seq = pdata->phy_init_seq;
			pr_info("USB parameters default!\n");
			break;
		...
		case OTG_STATE_A_HOST:
			seq = pdata->phy_init_seq_temp;
			pr_info("USB parameters second!\n");
			break;
...
static void msm_usb_phy_reset(struct msm_otg *motg)
{
	...
	pr_info("%s:start for USB parameters tag\n", __func__);
	switch (motg->pdata->phy_type) {
	case SNPS_PICO_PHY:
		pr_info("USB parameters tag phy_type is SNPS_PICO_PHY!\n");
		/* Assert USB PHY_PON */
		...
		break;
	case QUSB_ULPI_PHY:
		pr_info("USB parameters tag phy_type is QUSB_ULPI_PHY!\n");
		ret = reset_control_assert(motg->phy_reset);
		if (ret) {
			pr_err("phy_reset_clk assert failed %d\n", ret);
			break;
		...
		case OTG_STATE_B_HOST:
			...
			break;
		case OTG_STATE_A_HOST:
			seq = motg->pdata->phy_init_seq_temp;
			pr_info("USB parameters second!\n");
			break;
		case OTG_STATE_A_WAIT_VRISE:
			seq = motg->pdata->phy_init_seq_temp;
		...
		break;
	case SNPS_FEMTO_PHY:
		pr_info("USB parameters tag phy_type is SNPS_FEMTO_PHY!\n");
		if (!motg->phy_por_clk) {
...
static void msm_otg_start_peripheral(struct usb_otg *otg, int on)
{
	...
	motg->start_peripheral = !!on;
	dual_role_instance_changed(motg->dual_role);
}
...
static int msm_otg_read_usbid_voltage(struct msm_otg *motg)
{
	...
	struct device_node *np = motg->phy.dev->of_node;
	u32 channel_num;

	if (IS_ERR_OR_NULL(motg->otg_vadc_dev)) {
		motg->otg_vadc_dev = qpnp_get_vadc(motg->phy.dev, "usbidvol");
		if (IS_ERR(motg->otg_vadc_dev)) {
			pr_debug("adc not register,default value used usbid_mv=%d\n", usbid_mv);
			return usbid_mv;
		}
	}

	rc = of_property_read_u32(np, "adc,channel_num", &channel_num);
	if (rc && (rc != -EINVAL))
		pr_err("adc channel_num read fail \n");

	rc = qpnp_vadc_read(motg->otg_vadc_dev, channel_num, &results);
	if (rc) {
		pr_err("Unable to read usbid_mv default value used rc=%d\n", rc);
		motg->otg_vadc_dev = NULL;
		return usbid_mv;
	}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_IDLE:
		if (!test_bit(ID, &motg->inputs) && otg->host) {
			...
			pr_info("USB parameters b_idle to a_host!\n");
			...
			msm_otg_dbg_log_event(phy, "RT: Cable DISC",
				get_pm_runtime_counter(dev), 0);
			/* workaround: charger rerun notify usb disconnect, setting 0 will disconnect charge */
...
static void msm_id_status_w(struct work_struct *w)
{
	...
	if (motg->pdata->pmic_id_irq)
		...
		motg->id_state = motg->id_pin_state;
		if (motg->id_state) {
			mod_timer(&otg_disable_timer, jiffies + OTG_DISABLE_DELAY_TIME);
			printk(KERN_ALERT "usb20_host add_timer \n");
		} else {
			del_timer_sync(&otg_disable_timer);
			printk(KERN_ALERT "usb20_host del_timer \n");
		}
	}
	if (motg->err_event_seen)
		return;
...
struct msm_otg_platform_data *msm_otg_dt_to_pdata(struct platform_device *pdev)
{
	...
	len2 = of_property_count_elems_of_size(node, 
			  "qcom,hsusb-otg-phy-init-temp-seq", sizeof(len2));
	if (len2 > 0) {
		pdata->phy_init_seq_temp = devm_kzalloc(&pdev->dev, 
						len2 * sizeof(len2), GFP_KERNEL);
		if (!pdata->phy_init_seq_temp)
			return NULL;
		of_property_read_u32_array(node, "qcom,hsusb-otg-phy-init-temp-seq",
				pdata->phy_init_seq_temp, len2);
	}
	...
	if (pdata->usbid_adc_used) {
		...
		pr_debug("usbid_adc_used is available, ADC mode\n");
	} else
		pr_debug("usbid_adc_used is available, no ADC mode\n");
		...
		struct device_attribute *attr, char *buf)
{
	...
	case OTG_STATE_A_WAIT_BCON:
		...
		printk(KERN_ALERT "---otg enter host mode...\n");
		break;
	case OTG_STATE_B_PERIPHERAL:
		size = snprintf(buf, PAGE_SIZE, "%s", "peripheral\n");
		printk(KERN_ALERT "---otg enter peripheral mode...\n");
		break;
	default:
		size = snprintf(buf, PAGE_SIZE, "%s", "none\n");
		printk(KERN_ALERT "---otg enter none mode...\n");
		break;
	}
		...
		struct device_attribute *attr, char *buf)
{
	ssize_t size = 0;
	struct msm_otg *motg;
	int usbid_mv, total_mv = 0, i = 0, gpio_state = 0;
	motg = dev_get_drvdata(dev);
	...
	if (motg->pdata->usbid_adc_volt_gpio) {
		gpio_state = gpio_get_value(motg->pdata->usbid_adc_volt_gpio);
		gpio_direction_output(motg->pdata->usbid_adc_volt_gpio, 1);
	}
	msleep(100);
	...
	pr_debug("usbid_mv value: %d, gpio_state: %d\n", usbid_mv, gpio_state);
	if (motg->pdata->usbid_adc_volt_gpio && !gpio_state)
		gpio_direction_output(motg->pdata->usbid_adc_volt_gpio, 0);
...
static void otg_disable_func(unsigned long data)
{
	...
	printk(KERN_ALERT "add otg_disable_func work! \n");
}
static void host_mode_enable(struct msm_otg *motg, bool enabled)
{
	...
	if (!pdata->usb_id_gpio) {
		printk(KERN_ALERT "usb_id_gpio is not exist! \n");
	}
	if (host_mode_enabled ^ enabled) {
		...
		if (enabled) {
			if (motg->pdata->usb_id_gpio > 0) {
				...
				printk(KERN_ALERT "usbid adc used init otg_switch_disable_timer \n");
			}
		} else {
			...
			if (motg->pdata->usbid_adc_used) {
				...
				del_timer_sync(&otg_disable_timer);
				printk(KERN_ALERT "usb phy otg del otg_switch_disable_timer \n");
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	if (motg->pdata->mode == USB_OTG &&
		...
		!motg->phy_irq) {
		/*vivo add otg adc work start*/
		if (gpio_is_valid(motg->pdata->usbid_adc_volt_gpio)) {
			/* usbid_adc_gpio request */
			ret = devm_gpio_request(&pdev->dev, 
							motg->pdata->usbid_adc_volt_gpio,
							"USBID_ADC_VOLT_GPIO");
			if (ret < 0) {
				dev_err(&pdev->dev, "usbid_adc_volt_gpio req failed for usb_id_adc_volt\n");
				motg->pdata->usbid_adc_volt_gpio = 0;
				goto phy_reg_deinit;
			}
		}
		/*vivo add otg adc work end*/
	...
	if (motg->pdata->otg_control == OTG_PMIC_CONTROL &&
			(!(motg->pdata->mode == USB_OTG) ||
			 motg->pdata->pmic_id_irq || motg->ext_id_irq ||
								!motg->phy_irq || motg->pdata->usbid_adc_used))
	...
	if (device_create_file(&pdev->dev, &dev_attr_usbid_value)) {
		pr_err("Failed to create usbid_value file");
	}
...
static int msm_otg_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_usbid_value);
	device_remove_file(&pdev->dev, &dev_attr_usb_type);
