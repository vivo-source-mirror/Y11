...
/*Begin leiweiqiang add vivo's customization 2017-02-16*/
__attribute__((unused)) static void set_dload_mode_bbk(int on)
{
	...
	if (dload_mode_addr) {
		...
		__raw_writel(on ? 0xCE14091A : 0,
			dload_mode_addr + sizeof(unsigned int));
		*/
	#if !defined(VIVO_BOOT_MINI_RAM_DUMP_MAGIC_NUM)
		#define VIVO_BOOT_MINI_RAM_DUMP_MAGIC_NUM	0x44524D56
	#endif

		/*cookie->vivo_ram_dump_magic*/
		__raw_writel((u32)(on ? VIVO_BOOT_MINI_RAM_DUMP_MAGIC_NUM : 0),
			dload_mode_addr + 17*sizeof(u32));
		/*cookie_ptr->etb_buf_addr, 64bit*/
		__raw_writeq(__pa(log_buf_addr_get()),
			dload_mode_addr + 2*sizeof(u32));
		/*cookie_ptr->first_log, 64bit*/
		__raw_writeq(__pa(&log_first_idx),
			dload_mode_addr + 21*sizeof(u32));
		/*cookie_ptr->next_log, 64bit*/
		__raw_writeq(__pa(&log_next_idx),
			dload_mode_addr + 23*sizeof(u32));
		/*cookie_ptr->log_buf_len, 32bit*/
		__raw_writel(log_buf_len_get(),
			dload_mode_addr + 25*sizeof(u32));
		mb();
	}

	{
		int ret = scm_set_dload_mode(on ? (dload_type ? dload_type : SCM_DLOAD_FULLDUMP) : 0, 0);
		if (ret)
			pr_err("Failed to set secure DLOAD mode: %d\n", ret);
	}
	...
	*/
	...
	/*The dload_mode_enabled determine the state of sdi*/
	dload_mode_enabled = !!on;
}
/*End leiweiqiang add vivo's customization 2017-02-16*/
...
/*Begin leiweiqiang add vivo's customization 2017-02-16*/
#define set_dload_mode_bbk(on) do {} while (0)
/*End leiweiqiang add vivo's customization 2017-02-16*/
...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		...
		//vivo miaoqiang add for silent & survival reboot start
		...
		//vivo miaoqiang add for silent & survival reboot end
