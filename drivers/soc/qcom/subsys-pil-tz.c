...
struct work_struct subsys_crash_work;
char subsys_name[MAX_SUBSYS_NAME_LEN];
char subsys_crash_reason[MAX_SSR_REASON_LEN];
...
static void subsys_crash_collect(struct work_struct *work)
{
	...
	for (i = 0; i < subtype_id; i++) {
		if (strncmp(subtype_name[i], subsys_name, strlen(subtype_name[i])) == 0) {
			subtype_id = i + 1;
			break;
		}
	}
	//scnprintf(pos, 200, "%d\",\"times\":\"1\",\"exInfo\":\"%s:%s\"}]", subtype_id, subsys_name, subsys_crash_reason);
	scnprintf(pos, 200, "extype:1&subtype:%d&exInfo:%s:%s", subtype_id, subsys_name, subsys_crash_reason);
	pr_err("%s - %s subsystem failure ex_data: %s.\n", __func__, subsys_name, ex_data);
...
static void log_failure_reason(const struct pil_tz_data *d)
{
	...
	/* vivo marongqiang add for subsystem crash collect start */
	memset(subsys_name, '\0', MAX_SUBSYS_NAME_LEN);
	strlcpy(subsys_name, name, MAX_SUBSYS_NAME_LEN);
	memset(subsys_crash_reason, '\0', MAX_SSR_REASON_LEN);
	...
	if (!smem_reason || !size) {
		pr_err("%s SFR: (unknown, smem_get_entry_no_rlock failed).\n",
									name);
		scnprintf(subsys_crash_reason, MAX_SSR_REASON_LEN, "%s SFR: (unknown, smem_get_entry_no_rlock failed).", name);
		schedule_work(&subsys_crash_work);
		return;
	}
	if (!smem_reason[0]) {
		pr_err("%s SFR: (unknown, empty string found).\n", name);
		scnprintf(subsys_crash_reason, MAX_SSR_REASON_LEN, "%s SFR: (unknown, empty string found).", name);
		schedule_work(&subsys_crash_work);
		return;
	}
	...
	strlcpy(subsys_crash_reason, smem_reason, min(size, MAX_SSR_REASON_LEN));
	schedule_work(&subsys_crash_work);
	/* vivo marongqiang add for subsystem crash collect end */
}
