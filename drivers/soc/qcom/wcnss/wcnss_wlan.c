...
// vivo lanyonghui add for compatible with multiple calibration scheme begin
char bin_bias = -1;
// vivo lanyonghui add for compatible with multiple calibration scheme end
...
static int wcnss_ready;
module_param(wcnss_ready, int, 0644);
MODULE_PARM_DESC(wcnss_ready, "vivo add for BT FTM test.");
...
// vivo lanyonghui add for compatible with multiple calibration scheme begin
#define WCNSS_USR_SPECIFIED_WLAN_BIN_FILE   (WCNSS_USR_CTRL_MSG_START + 4)
// vivo lanyonghui add for compatible with multiple calibration scheme end
...
// vivo lanyonghui add for compatible with multiple calibration scheme begin
#define NVBIN_FILE_BIAS0 "wlan/prima/WCNSS_qcom_wlan_nv.bin.BIAS0"
#define NVBIN_FILE_BIAS1 "wlan/prima/WCNSS_qcom_wlan_nv.bin.BIAS1"
#define NVBIN_FILE_SECOND "wlan/prima/WCNSS_qcom_wlan_nv_SECOND.bin"
#define NVBIN_FILE_POWERDEFAULT "wlan/prima/WCNSS_qcom_wlan_nv.bin.POWERDEFAULT"
#define NVBIN_FILE_POWERLOW "wlan/prima/WCNSS_qcom_wlan_nv.bin.POWERLOW"
#define NVBIN_FILE_POWERHIGH "wlan/prima/WCNSS_qcom_wlan_nv.bin.POWERHIGH"
#define NVBIN_FILE_SECONDCOUPLER "wlan/prima/WCNSS_qcom_wlan_nv.bin.SECONDCOUPLER"
#define NVBIN_FILE_WCN3680 "wlan/prima/WCNSS_qcom_wlan_nv.bin.WCN3680"
#define NVBIN_FILE_WCN3615 "wlan/prima/WCNSS_qcom_wlan_nv.bin.WCN3615"
// vivo lanyonghui add for compatible with multiple calibration scheme end
...
unsigned int wcnss_get_serial_number(void)
{
	if (penv) {
		...
		wcnss_log(INFO, "%s: Device serial number: %u, %0x\n",
			__func__, penv->serial_number, penv->serial_number);
...
static void wcnss_process_smd_msg(int len)
{
	...
	case WCNSS_CBC_COMPLETE_IND:
		penv->is_cbc_done = 1;
		wcnss_log(ERR, "received WCNSS_CBC_COMPLETE_IND from FW\n");
		wcnss_ready = 1;   /* add by vivo liudingrong*/
		break;
...
extern char *get_bbk_board_name(void);
...
void wcnss_get_board_info(void)
{
	...
	board_name = get_bbk_board_name();
	board_version = get_bbk_board_version();

	if (!board_name || !board_version) {
		pr_err("wcnss: %s: board_name or board_version get null, use default bin file\n", __func__);
	...
	pr_err("wcnss: %s: board_name is %s, board_version %s\n", __func__, board_name, board_version);

	if (!strncmp(board_name, "PD1930", 6)) {
		pr_err("wcnss: %s: %d, PD1930 Project Matched, continue check board_version and BIAS", __func__, __LINE__);
		if ('0' == board_version[4]) {
			pr_err("wcnss: %s: %d, without coupler the bin_bias = %d", __func__, __LINE__, bin_bias);
			memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
			switch (bin_bias) {
			case 0:
					pr_err("wcnss: %s: %d, PD1930 bias bin file 0 nv", __func__, __LINE__);
					size_bin = strlen(NVBIN_FILE_BIAS0);
					memcpy(penv->nv_bin_file, NVBIN_FILE_BIAS0, size_bin);
					break;
			case 1:
					pr_err("wcnss: %s: %d, PD1930 bias bin file 1 nv", __func__, __LINE__);
					size_bin = strlen(NVBIN_FILE_BIAS1);
					memcpy(penv->nv_bin_file, NVBIN_FILE_BIAS1, size_bin);
					break;
			case 2:
					pr_err("wcnss: %s: %d, PD1930 power bin file low nv", __func__, __LINE__);
					size_bin = strlen(NVBIN_FILE_POWERLOW);
					memcpy(penv->nv_bin_file, NVBIN_FILE_POWERLOW, size_bin);
					break;
			case 3:
					pr_err("wcnss: %s: %d, PD1930 power bin file default nv", __func__, __LINE__);
					size_bin = strlen(NVBIN_FILE_POWERDEFAULT);
					memcpy(penv->nv_bin_file, NVBIN_FILE_POWERDEFAULT, size_bin);
					break;
			case 4:
					pr_err("wcnss: %s: %d, PD1930 power bin file high nv", __func__, __LINE__);
					size_bin = strlen(NVBIN_FILE_POWERHIGH);
					memcpy(penv->nv_bin_file, NVBIN_FILE_POWERHIGH, size_bin);
					break;
			default:
					pr_err("wcnss: %s: %d, without coupler but bias value is wrong. so use PD1930 second default nv", __func__, __LINE__);
					size_bin = strlen(NVBIN_FILE_SECOND);
					memcpy(penv->nv_bin_file, NVBIN_FILE_SECOND, size_bin);
			}
		} else {
			pr_err("wcnss: %s: %d, with coupler continue check coupler provider", __func__, __LINE__);
			memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
			if ('1' == board_version[6]) {
				pr_err("wcnss: %s: %d, 1st provider use PD1930 default nv", __func__, __LINE__);
				size_bin = strlen(NVBIN_FILE);
				memcpy(penv->nv_bin_file, NVBIN_FILE, size_bin);
			} else if ('0' == board_version[6]) {
				pr_err("wcnss: %s: %d, 2nd provider use PD1930 second coupler nv", __func__, __LINE__);
				size_bin = strlen(NVBIN_FILE_SECONDCOUPLER);
				memcpy(penv->nv_bin_file, NVBIN_FILE_SECONDCOUPLER, size_bin);
			} else {
				pr_err("wcnss: %s: %d, neither 1st nor 2nd provider, so use default nv", __func__, __LINE__);
				size_bin = strlen(NVBIN_FILE);
				memcpy(penv->nv_bin_file, NVBIN_FILE, size_bin);
			}
		}
	} else if (!strncmp(board_name, "PD2060", 6)) {
		pr_err("wcnss: %s: %d, PD2060 Project Matched, continue check board_version", __func__, __LINE__);
		/*
		GPIO 133 == 1 && GPIO 97 == 1  -->  WCN3660B
		GPIO 133 == 0 && GPIO 97 == 1  -->  WCN3680
		GPIO 133 == 1 && GPIO 97 == 0  -->  WCN3615 only support 2.4g
		*/
		if ('1' == board_version[8] && '1' == board_version[9]) {
			pr_err("wcnss: %s: %d, WCN3660B matched.", __func__, __LINE__);
			size_bin = strlen(NVBIN_FILE_BIAS1);
			memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
			memcpy(penv->nv_bin_file, NVBIN_FILE_BIAS1, size_bin);
		} else if ('0' == board_version[8] && '1' == board_version[9]) {
			pr_err("wcnss: %s: %d, WCN3680 matched.", __func__, __LINE__);
			size_bin = strlen(NVBIN_FILE_WCN3680);
			memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
			memcpy(penv->nv_bin_file, NVBIN_FILE_WCN3680, size_bin);
		} else if ('1' == board_version[8] && '0' == board_version[9]) {
			pr_err("wcnss: %s: %d, WCN3615 matched.", __func__, __LINE__);
			size_bin = strlen(NVBIN_FILE_WCN3615);
			memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
			memcpy(penv->nv_bin_file, NVBIN_FILE_WCN3615, size_bin);
		} else {
			pr_err("wcnss: %s: %d, no WCN chips matched, use the default bin file", __func__, __LINE__);
			size_bin = strlen(NVBIN_FILE);
			memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
			memcpy(penv->nv_bin_file, NVBIN_FILE, size_bin);
		}
	} else {
		pr_err("wcnss: %s: %d, No Project Matched, use default bin file", __func__, __LINE__);
		size_bin = strlen(NVBIN_FILE);
		memset(penv->nv_bin_file, 0, sizeof(penv->nv_bin_file));
		memcpy(penv->nv_bin_file, NVBIN_FILE, size_bin);
	}
...
static void wcnss_nvbin_dnld(void)
{
	...
	if (ret || !nv || !nv->data || !nv->size) {
		wcnss_log(ERR,
			"%s: request_firmware failed for %s, use default %s \n", __func__,
...
void process_usr_ctrl_cmd(u8 *buf, size_t len)
{
	...
	case WCNSS_USR_WLAN_MAC_ADDR:
		...
		break;
	// vivo lanyonghui add for compatible with multiple calibration scheme begin
	case WCNSS_USR_SPECIFIED_WLAN_BIN_FILE:
		bin_bias = buf[2];
		wcnss_log(ERR, "%s: expect BIAS = %d\n", __func__, bin_bias);
		break;
	// vivo lanyonghui add for compatible with multiple calibration scheme begin
	default:
