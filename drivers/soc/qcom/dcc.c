		...
		struct device_node *np)
{
	...
	if (prop) {
		...
		i = 0;
		while (i < len) {
			addr = be32_to_cpu(prop[i++]);
			size = be32_to_cpu(prop[i++]);
			ret = dcc_config_add(drvdata, addr, size);
			if (ret) {
				dev_err(drvdata->dev,
...
static ssize_t dcc_enable_from_kernel(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (val) {
		dcc_config_reset(drvdata);
		dcc_configure_list(drvdata, dev->of_node);
	} else
		dcc_disable(drvdata);
	return size;
}
static DEVICE_ATTR(dcc_enable_kernel, 0200, NULL, dcc_enable_from_kernel);
