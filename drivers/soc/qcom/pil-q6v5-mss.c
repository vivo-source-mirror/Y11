...
extern struct work_struct subsys_crash_work;
extern char subsys_name[MAX_SUBSYS_NAME_LEN];
extern char subsys_crash_reason[MAX_SSR_REASON_LEN];
...
static void log_modem_sfr(void)
{
	...
	strlcpy(subsys_name, "modem", MAX_SUBSYS_NAME_LEN);
	...
	if (!smem_reason || !size) {
		...
		strlcpy(subsys_crash_reason, "modem subsystem failure reason: (unknown, smem_get_entry_no_rlock failed).", MAX_SSR_REASON_LEN);
		schedule_work(&subsys_crash_work);
		return;
	}
	if (!smem_reason[0]) {
		pr_err("modem subsystem failure reason: (unknown, empty string found).\n");
		strlcpy(subsys_crash_reason, "modem subsystem failure reason: (unknown, empty string found).", MAX_SSR_REASON_LEN);
		schedule_work(&subsys_crash_work);
		return;
	}
