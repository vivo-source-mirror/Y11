...
int track_event(const char *module_id, const char *event_id, const char *params)
{
	...
	if (!module_id || !event_id || !params || strlen(params) < 1) {
		printk(KERN_ERR "track_event invlid arguments");
		return -EINVAL;
	}
	...
	entry = kzalloc(sizeof(struct event_track_entry), GFP_KERNEL);
	if (!entry) {
		err = -ENOMEM;
		goto out;
	}
	...
	ENTRY_LEN_MAX, "%s&%s&%s\n", module_id, event_id, params);

	spin_lock_irq(&gp_event_track->lock);
	err = event_track_write_entry(gp_event_track, entry);
	spin_unlock_irq(&gp_event_track->lock);
	if (err)
		kfree(entry);
	else
		wake_up_interruptible(&gp_event_track->event_track_waitq);
out:
	return err;
}
...
{
	va_list args;
	char params[DATA_LEN_MAX];

	memset(params, 0, sizeof(params));
	va_start(args, fmt);
	vscnprintf(params, DATA_LEN_MAX, fmt, args);
	va_end(args);
	return track_event(module_id, event_id, params);
}
